...
pubDate: 2026-05-13T14:00:00+08:00
updatedDate: 2026-05-13T14:00:00+08:00
...
Phase 5 的目標是把整個流程自動化，建立一個完整的 **MLOps 閉環**（如圖 1 所示）：
...
![MLOps 全景閉環架構圖](../../../../assets/images/2026/05/mlops_phase5/mlops_phase5-1.webp)
<p style="text-align: center; font-size: 0.875rem; color: #6b7280;">圖1 ▲ MLOps 完整閉環：從 git push 觸發 CI/CD，經 SageMaker Pipeline 訓練、Model Registry 審核、自動部署，到 CloudWatch 監控與異常重訓</p>
...
![Model Registry 版本管控視圖](../../../../assets/images/2026/05/mlops_phase5/mlops_phase5-2.webp)
<p style="text-align: center; font-size: 0.875rem; color: #6b7280;">圖2 ▲ Model Registry 版本管控：每次訓練產生新版本，記錄 accuracy/F1 等指標，透過 Approved/Rejected/PendingManualApproval 狀態控管部署流程</p>

### 第二步：SageMaker Pipeline — 自動化訓練→登記

Pipeline 把各個步驟串成有向圖（DAG）（如圖 3 所示），每次執行時按順序跑：
...
![SageMaker Pipeline DAG 有向無環圖](../../../../assets/images/2026/05/mlops_phase5/mlops_phase5-3.webp)
<p style="text-align: center; font-size: 0.875rem; color: #6b7280;">圖3 ▲ SageMaker Pipeline DAG：Processing → Training → RegisterModel 各 Step 的依賴關係，以及每個 Step 產出的 Artifact 流向</p>
...
![公開 API 存取架構圖（Serverless 推論）](../../../../assets/images/2026/05/mlops_phase5/mlops_phase5-4.webp)
<p style="text-align: center; font-size: 0.875rem; color: #6b7280;">圖4 ▲ Serverless 公開 API 架構：外部請求經 API Gateway → Lambda 轉發鑑權 → SageMaker Endpoint 推論，讓私有 Endpoint 安全對外服務</p>
...
![CloudWatch 監控與警報儀表板](../../../../assets/images/2026/05/mlops_phase5/mlops_phase5-5.webp)
<p style="text-align: center; font-size: 0.875rem; color: #6b7280;">圖5 ▲ CloudWatch 監控儀表板：延遲（P50/P90/P99）、每秒呼叫次數、CPU 使用率，搭配 Alarm 觸發 SNS 通知的完整警報鏈路</p>
